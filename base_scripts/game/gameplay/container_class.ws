...
class CContainer extends CGameplayEntity {
	...
	// AUTO-LOOT BEGIN
	editable var autoLootWhenNearby : bool;
	// AUTO-LOOT END
	...
	// AUTO-LOOT BEGIN
	default autoLootWhenNearby = true;
	// AUTO-LOOT END
		...
		if (IsLootable()) {
			if (isDynamic) {
				...
				for (i = allItems.Size() - 1; i >= 0; i -= 1) {
					itemId = allItems[i];
					if (
						sourceInv.GetItemAttributeAdditive(itemId, 'item_weight') == 0 &&
						!sourceInv.ItemHasTag(itemId, 'Quest')
					) {
...
event OnInteractionActivated(interactionName : name, activator : CEntity) {
    // ============ DECLARE ALL VARIABLES HERE AT THE VERY TOP ============
    var hasQuestItem : bool = false;
...
//			&& (!lockedByKey || thePlayer.GetInventory().HasItem(keyItemName))
		...
		{
            // --- Check if this container has a quest item ---
            inv.GetAllItems(allItems);
            for (i = 0; i < allItems.Size(); i += 1) {
                if (inv.ItemHasTag(allItems[i], 'Quest')) {
                    hasQuestItem = true;
                    break;
                }
            }
            ...
            // AUTO-LOOT BEGIN
            // Only auto-loot if there are NO quest items inside
            if (
                autoLootWhenNearby &&
                VecDistance2D(this.GetWorldPosition(), thePlayer.GetWorldPosition()) <= 5.0 &&
                !hasQuestItem
            ) {
                TakeAllItems();
                QuestItemGlow();
                ...
                ShowLootPreview(); // Keeps container alive so full loot window can open
            }
            // AUTO-LOOT END
        }
    }
	...
	function IsLootable() : bool {
		...
		for (i = 0; i < allItems.Size(); i += 1) {
			if (
				!GetInventory().ItemHasTag(allItems[i], 'NoDrop') &&
				GetInventory().GetItemQuantity(allItems[i]) > 0
			) {
				isAnything = true;
			}
		}
	...
	event OnInteraction(actionName : name, activator : CEntity) {
		...
		if (actionName != 'Container') return false;
	...
	function ShowLootPreview() {
		...
		for (i = allItems.Size() - 1; i >= 0; i -= 1) // questowe
		{
			...
			if (!itemTags.Contains('NoDrop') && itemTags.Contains('Quest')) {
				...
				theHud.SetString(
					"Icon",
					"img://globals/gui/icons/items/" + StrReplaceAll(itemName, " ", "") + "_64x64.dds",
					AS_item
				);
...
function Helper_TransferItemFromContainerToPlayer(
	itemId : SItemUniqueId,
	targetInv, sourceInv : CInventoryComponent,
	isDynamic : bool
) {
	var quantity : int;
	var itemName : name;
...
exec function LootItem(itemName : string, quantity : int) {
	...
	if (thePlayer.GetLastContainer()) {
		...
		if (AllowItemDarkDiff(thePlayer.GetLastContainer().GetInventory(), itemId)) {
			Helper_TransferItemFromContainerToPlayer(
				itemId,
				thePlayer.GetInventory(),
				thePlayer.GetLastContainer().GetInventory(),
				thePlayer.GetLastContainer().isDynamic
			);
		}
...
function CheckForTutorial(itemId : SItemUniqueId, sourceInv : CInventoryComponent) {
	...
	if (sourceInv.GetItemCategory(itemId) == 'armorupgrade') {
...
function AllowItemDarkDiff(sourceInv : CInventoryComponent, itemId : SItemUniqueId) : bool {
	if (
		(sourceInv.ItemHasTag(itemId, 'DarkDiffA1') || sourceInv.ItemHasTag(itemId, 'DarkDiffA2') || sourceInv.ItemHasTag(itemId, 'DarkDiffA3') || sourceInv.ItemHasTag(itemId, 'DarkDiff')) &&
		theGame.GetDifficultyLevel() < 4
	) {
		return false;
	}
	else {
