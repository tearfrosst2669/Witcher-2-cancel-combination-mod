...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	event OnGameInputEvent( key : name, value : float )
	{
		...
		// Dodge cancel — intercept before attack handling
    if ( key == 'GI_Accept_Evade' && value > 0.5f
         && !parent.IsOverweight()
         && !parent.AreCombatHotKeysBlocked()
         && (!thePlayer.isNotGeralt || thePlayer.IsAssasinReplacer()) )
    {
        DodgeCancelSilver();
        return true;
    }
	...
	private function HandleAttackInput(key : name, value : float) : bool
	{
	 ...
	 // Attack cancels dodge
    if(parent.IsDodgeing() && value > 0.5f)
    {
        if(IsKeyAttackFast(key) || IsKeyAttackStrong(key))
        {
            AttackFromDodgeSilver();
            return true;
        }
    }
	...
	entry function DodgeCancelSilver()
{
    parent.ActionCancelAll();
    parent.SetPlayerEvadeType(parent.GetPlayerEvadeType());
    parent.PlayerActionUnbreakableForced(PAU_Evade);
    parent.SetPlayerCombatStance(PCS_High);
    LoopCombatSilver();
}

	entry function AttackFromDodgeSilver()
{
    var enemy : CActor;
    ...
    if(ComboAttackFast(GetAttackDirection(), GetAttackDistance()))
    {
        parent.RegisterActiveAction('AttackEnd');
    }
